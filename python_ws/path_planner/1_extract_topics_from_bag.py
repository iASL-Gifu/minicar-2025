...
def extract_and_save_per_bag(
    bag_path: Path, output_dir: Path, image_topic: str, odom_topic: str,
    prediction_horizon: int, time_step_s: float,
    history_length: int, history_step_s: float # <--- 追加: 過去履歴用の引数
):
    """
        ...
        with AnyReader([bag_path]) as reader:
            ...
            for conn, timestamp, raw in reader.messages(connections=connections):
                ...
                if conn.topic == image_topic and conn.msgtype == 'sensor_msgs/msg/Image':
                    ...
                    if encoding == 'mono8': shape = (msg.height, msg.width)
                    elif encoding in ['bgr8', 'rgb8']: shape = (msg.height, msg.width, 3)
                    else: continue
    ...
    synced_images = []
    synced_past_odoms = [] # <--- 追加: 過去のOdometry履歴を保存するリスト
    ...
    for i, itime in enumerate(image_times):
        ...
        for k in range(1, prediction_horizon + 1):
            future_time = itime + (k * time_step_s * 1e9)
            # 現在時刻以降のオドメトリから最も近いものを探す
            search_indices = np.where(odom_times >= itime)[0]
            if len(search_indices) == 0:
                is_future_path_valid = False; break
        ...
        for k in range(history_length):
            past_time = itime - (k * history_step_s * 1e9)
            # 現在時刻以前のオドメトリから最も近いものを探す
            search_indices = np.where(odom_times <= itime)[0]
            if len(search_indices) == 0:
                is_history_valid = False; break
            
            relative_idx = np.argmin(np.abs(odom_times[search_indices] - past_time))
            absolute_idx = search_indices[relative_idx]
            past_odom_indices.append(absolute_idx)

        # 未来パス、過去履歴のどちらかが不完全な場合はスキップ
        if not is_future_path_valid or not is_history_valid or \
           len(future_path_indices) != prediction_horizon or \
           len(past_odom_indices) != history_length:
            continue
    ...
    np.save(out_dir / 'past_odoms.npy', np.array(synced_past_odoms, dtype=np.float32)) # <--- 追加
...
def main():
    ...
    parser.add_argument('--horizon', type=int, default=10,
                        help='(Future) Number of future trajectory points to generate.')
    parser.add_argument('--time-step', type=float, default=0.1,
                        help='(Future) Time step in seconds between future trajectory points.')
    parser.add_argument('--history-len', type=int, default=5,
                        help='(Past) Number of past odometry points to retrieve.')
    parser.add_argument('--history-step', type=float, default=0.1,
                        help='(Past) Time step in seconds between past odometry points.')
    ...
    for bag_path in bag_paths:
        extract_and_save_per_bag(
            ...
            history_step_s=args.history_step
        )
